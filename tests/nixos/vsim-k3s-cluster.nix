...
#   └── Inner VMs (libvirt):
#       ├── server-1 (k3s server) - 192.168.100.10
#       ├── server-2 (k3s server) - 192.168.100.11
#       └── agent-1 (k3s agent)  - 192.168.100.12
#
# TEST PHASES:
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # Inner VM IPs
    VMS = {
        "server-1": "192.168.100.10",  # k3s server (cluster init)
        "server-2": "192.168.100.11",  # k3s server (joins)
        "agent-1": "192.168.100.12",  # k3s agent
    }
    ...
    # Start server-1 first (it's the cluster init server)
    tlog("  Starting server-1 (primary k3s server)...")
    emulator.succeed("virsh start server-1")
    tlog("  ✓ Started server-1")

    # Wait for server-1 to get network before starting others
    tlog("  Waiting 10s for server-1 to initialize...")
    time.sleep(10)

    # Start remaining VMs
    tlog("  Starting server-2...")
    emulator.succeed("virsh start server-2")
    tlog("  ✓ Started server-2")
    tlog("  Starting agent-1...")
    emulator.succeed("virsh start agent-1")
    tlog("  ✓ Started agent-1")
    ...
    # Wait for k3s.service on server-1 (primary server)
    tlog(f"  Waiting for k3s.service on server-1 (max {K3S_SERVICE_ATTEMPTS} attempts)...")
    for attempt in range(K3S_SERVICE_ATTEMPTS):
        ...
        if exit_code == 0:
            tlog(f"  ✓ k3s.service active on server-1 after {attempt * 2} seconds")
            break
        time.sleep(2)
    else:
        # Debug output
        ...
        raise Exception("k3s.service did not start on server-1")
    ...
    for attempt in range(KUBECTL_ATTEMPTS):
        ...
        if exit_code == 0 and "server-1" in output:
            tlog("  ✓ kubectl working, cluster accessible")
            break
        time.sleep(2)
    else:
        raise Exception("kubectl not working on server-1")
    ...
    assert "server-1" in nodes_output, "server-1 not in kubectl output"
    assert "Ready" in nodes_output, "No nodes are Ready"
    tlog("  ✓ Primary server (server-1) is Ready")

    # Check if other nodes joined
    if "server-2" in nodes_output:
        tlog("  ✓ server-2 joined the cluster")
    if "agent-1" in nodes_output:
