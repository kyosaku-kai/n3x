...
mkK3sClusterTest {
  ...
  testScript = ''
    ...
    for node, name in [(server_1, "server-1"), (server_2, "server-2"), (agent_1, "agent-1")]:
        # Verify bond0 exists
    ...
    tlog("  server-1 k3s.service started")
    ...
    pre_nodes = server_1.succeed("k3s kubectl get nodes --no-headers").strip()
    ...
    # PHASE 5: Simulate Primary NIC Failure on server-1
    tlog("\n[PHASE 5] Simulating eth1 failure on server-1...")

    # Record active slave before failure
    active_before = get_active_slave(server_1)
    tlog(f"  Active slave before failure: {active_before}")

    # Bring down primary interface
    server_1.succeed("ip link set eth1 down")
    ...
    active_after = get_active_slave(server_1)
    ...
    bond_state = server_1.succeed("ip link show bond0")
    ...
    server_1.wait_until_succeeds("k3s kubectl get nodes", timeout=60)
    ...
    tlog("  server-1 still Ready after failover")

    nodes_during = server_1.succeed("k3s kubectl get nodes -o wide")
    tlog(f"  Nodes during failover:\n{nodes_during}")

    # PHASE 8: Restore Primary Interface
    tlog("\n[PHASE 8] Restoring eth1...")

    server_1.succeed("ip link set eth1 up")
    ...
    active_failback = get_active_slave(server_1)
    ...
    bond_status = server_1.succeed("cat /proc/net/bonding/bond0")
    ...
    final_nodes = server_1.succeed("k3s kubectl get nodes -o wide")
