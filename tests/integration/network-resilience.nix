...
#
# Inner VMs (n100-1, n100-2, n100-3) have pre-built NixOS images with k3s
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    for vm in ["n100-1", "n100-2", "n100-3"]:
        assert vm in virsh_list, "VM " + vm + " not defined in libvirt"
    print("  Inner VMs defined: n100-1, n100-2, n100-3")
    ...
    # Start n100-1 (it won't boot to an OS but will create a vnet interface)
    emulator.succeed("virsh start n100-1")
    time.sleep(3)  # Wait for interface to be created
    print("  Started n100-1")

    # Check that interface was created
    n100_1_iface = emulator.succeed("virsh domiflist n100-1 | grep -oP 'vnet\\d+' || echo 'none'").strip()
    if n100_1_iface != "none" and n100_1_iface:
        print("  n100-1 interface: " + n100_1_iface)
        ...
        emulator.succeed("/etc/tc-simulate-constraints.sh constrained")
        tc_show = emulator.succeed("tc qdisc show dev " + n100_1_iface)
        # tbf (token bucket filter) should be present for constrained profile
        if "tbf" in tc_show or "rate" in tc_show:
            print("  Constrained profile applied to " + n100_1_iface)
        else:
        ...
        tc_show = emulator.succeed("tc qdisc show dev " + n100_1_iface)
        if "netem" in tc_show:
            print("  Lossy profile applied to " + n100_1_iface)
        else:
        ...
        emulator.succeed("/etc/tc-simulate-constraints.sh default")
        tc_show = emulator.succeed("tc qdisc show dev " + n100_1_iface)
        print("  Rules cleared from " + n100_1_iface)

        # Status should now show the running VM
        status_output = emulator.succeed("/etc/tc-simulate-constraints.sh status")
        assert n100_1_iface in status_output, "Status should show running VM interface"
        print("  Status command shows running VM interface")
    else:
        print("  Warning: Could not detect vnet interface for n100-1")
        print("  This may be expected if QEMU interface creation is delayed")

    # Clean up - shutdown VM
    emulator.succeed("virsh destroy n100-1 || true")
    print("  Stopped n100-1")
