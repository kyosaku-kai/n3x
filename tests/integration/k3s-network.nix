...
#   - n100_1: k3s server (single server with SQLite)
#   - n100_2: k3s agent (worker)
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    tlog("  n100_1: k3s server @ ${network.n100_1}")
    tlog("  n100_2: k3s agent  @ ${network.n100_2}")
    tlog("  n100_3: k3s agent  @ ${network.n100_3}")
    tlog("Network CIDRs:")
    ...
    n100_1.wait_until_succeeds("k3s kubectl get --raw /readyz", timeout=180)
    ...
    n100_1.succeed("ping -c 2 ${network.n100_2}")
    tlog("  n100_1 -> n100_2: OK")
    n100_1.succeed("ping -c 2 ${network.n100_3}")
    tlog("  n100_1 -> n100_3: OK")

    n100_2.succeed("ping -c 2 ${network.n100_1}")
    tlog("  n100_2 -> n100_1: OK")
    n100_2.succeed("ping -c 2 ${network.n100_3}")
    tlog("  n100_2 -> n100_3: OK")

    n100_3.succeed("ping -c 2 ${network.n100_1}")
    tlog("  n100_3 -> n100_1: OK")
    n100_3.succeed("ping -c 2 ${network.n100_2}")
    tlog("  n100_3 -> n100_2: OK")

    tlog("  Node-to-node connectivity verified!")

    # Check flannel interfaces
    tlog("\n  Flannel network interfaces:")
    flannel_output = n100_1.succeed("ip -br addr show | grep -E 'flannel|cni'")
    tlog(f"  n100_1:\n{flannel_output}")

    # Show VXLAN configuration
    vxlan_check = n100_1.execute("ip -d link show flannel.1 2>/dev/null")
    if vxlan_check[0] == 0:
    ...
    tlog("  Testing DNS resolution from n100_1...")
    ...
    dns_result = n100_1.succeed(f"dig @{dns_svc_ip} kubernetes.default.svc.cluster.local +short")
    ...
    kube_dns_result = n100_1.succeed(
    ...
    ptr_check = n100_1.execute(f"dig @{dns_svc_ip} -x {k8s_api_ip} +short")
    ...
    n100_2.wait_until_succeeds(
        f"dig @{coredns_ip} kubernetes.default.svc.cluster.local +short | grep -E '^[0-9]'",
        timeout=60
    )
    tlog("  n100_2 -> DNS resolution via pod IP: OK")
    n100_3.wait_until_succeeds(
        f"dig @{coredns_ip} kubernetes.default.svc.cluster.local +short | grep -E '^[0-9]'",
        timeout=60
    )
    tlog("  n100_3 -> DNS resolution via pod IP: OK")
    ...
    tlog("  n100_2 -> DNS resolution via ClusterIP: OK")
    n100_3.wait_until_succeeds(
        f"dig @{dns_svc_ip} kubernetes.default.svc.cluster.local +short | grep -E '^[0-9]'",
        timeout=120
    )
    tlog("  n100_3 -> DNS resolution via ClusterIP: OK")
    ...
    iptables_nat = n100_1.succeed("iptables -t nat -L -n | head -30")
    tlog(f"  NAT rules (first 30 lines):\n{iptables_nat}")

    # Check for KUBE chains
    kube_chains = n100_1.succeed("iptables -t nat -L -n | grep -E '^(Chain KUBE|KUBE-)' | head -10")
    tlog(f"  KUBE chains:\n{kube_chains}")

    # Check CNI configuration
    tlog("\n  CNI configuration:")
    cni_config = n100_1.succeed("cat /var/lib/rancher/k3s/agent/etc/cni/net.d/*.conflist 2>/dev/null | head -50 || echo 'No CNI config found'")
    tlog(f"{cni_config}")

    # Verify NetworkPolicy CRD exists (k3s includes network policy controller)
    np_check = n100_1.execute("k3s kubectl api-resources | grep networkpolicies")
    if np_check[0] == 0:
    ...
    api_svc_ip = n100_1.succeed(
        "k3s kubectl get svc kubernetes -o jsonpath='{.spec.clusterIP}'"
    ).strip()
    n100_1.succeed(f"curl -k -s --max-time 5 https://{api_svc_ip}:443/healthz")
    tlog(f"  Service endpoint ({api_svc_ip}:443) -> /healthz: OK")

    # Test CoreDNS endpoints
    coredns_endpoints = n100_1.succeed(
    ...
    n100_1.succeed(f"dig @{dns_svc_ip} kubernetes.default.svc.cluster.local +short +time=5")
