...
#   - n100_1: k3s server (control plane)
#   - n100_2: k3s agent (worker 1)
#   - n100_3: k3s agent (worker 2)
#
# TEST PHASES:
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    baseline_1_2, loss_1_2 = measure_latency(n100_1, "${network.n100_2}")
    tlog(f"    n100_1 -> n100_2: {baseline_1_2:.2f}ms, {loss_1_2}% loss")
    baseline_1_3, loss_1_3 = measure_latency(n100_1, "${network.n100_3}")
    tlog(f"    n100_1 -> n100_3: {baseline_1_3:.2f}ms, {loss_1_3}% loss")
    baseline_2_3, loss_2_3 = measure_latency(n100_2, "${network.n100_3}")
    tlog(f"    n100_2 -> n100_3: {baseline_2_3:.2f}ms, {loss_2_3}% loss")
    ...
    # Apply 50ms latency with 10ms jitter to n100_2
    tlog("  Applying 50ms (+/-10ms) latency to n100_2 eth1...")
    apply_tc_netem(n100_2, "eth1", delay_ms=50, jitter_ms=10)

    tc_output = show_tc(n100_2, "eth1")
    assert "netem" in tc_output, "netem qdisc not applied"
    tlog(f"  TC config: {tc_output.strip()}")

    # Measure latency with constraints
    constrained_latency, _ = measure_latency(n100_1, "${network.n100_2}", count=10)
    tlog(f"  n100_1 -> n100_2 with constraints: {constrained_latency:.2f}ms")
    ...
    clear_tc(n100_2, "eth1")
    tlog("  Cleared latency constraint from n100_2")
    ...
    # Apply 5% packet loss to n100_3
    tlog("  Applying 5% packet loss to n100_3 eth1...")
    apply_tc_netem(n100_3, "eth1", loss_pct=5)

    tc_output = show_tc(n100_3, "eth1")
    assert "loss" in tc_output, "loss rule not applied"
    tlog(f"  TC config: {tc_output.strip()}")

    # Test with more pings to observe loss
    _, observed_loss = measure_latency(n100_1, "${network.n100_3}", count=50)
    tlog(f"  Observed packet loss to n100_3: {observed_loss}%")
    ...
    n100_1.wait_until_succeeds(
        "k3s kubectl get node n100-3 --no-headers | grep -w Ready",
        timeout=60
    )
    tlog("  n100-3 still Ready despite packet loss")

    # Clear packet loss
    clear_tc(n100_3, "eth1")
    tlog("  Cleared packet loss from n100_3")
    ...
    apply_tc_netem(n100_2, "eth1", rate_mbit=10)
    apply_tc_netem(n100_3, "eth1", rate_mbit=10)

    tc_2 = show_tc(n100_2, "eth1")
    tc_3 = show_tc(n100_3, "eth1")
    tlog(f"  n100_2 TC: {tc_2.strip()}")
    tlog(f"  n100_3 TC: {tc_3.strip()}")
    ...
    n100_1.succeed("k3s kubectl get pods -A")
    ...
    tlog("  Simulating edge device on n100_3: 20Mbps, 30ms latency, 1% loss...")
    apply_tc_netem(n100_3, "eth1", delay_ms=30, jitter_ms=5, loss_pct=1, rate_mbit=20)

    tc_output = show_tc(n100_3, "eth1")
    tlog(f"  TC config: {tc_output.strip()}")

    # Measure combined impact
    combined_latency, combined_loss = measure_latency(n100_1, "${network.n100_3}", count=20)
    ...
    recovered_latency, recovered_loss = measure_latency(n100_1, "${network.n100_3}", count=10)
