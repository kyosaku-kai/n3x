...
#   - n100_1: k3s server (single server with SQLite - more reliable in CI)
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    tlog("  n100_1: k3s server")
    tlog("  n100_2: k3s agent (worker 1)")
    tlog("  n100_3: k3s agent (worker 2)")
    tlog("=" * 70)
    ...
    for node_name, node in [("n100_1", n100_1), ("n100_2", n100_2), ("n100_3", n100_3)]:
        tlog(f"\n  Checking {node_name}...")
    ...
    sc_output = n100_1.succeed("k3s kubectl get storageclass local-path -o jsonpath='{.metadata.name}'")
    ...
    sc_details = n100_1.succeed("k3s kubectl get storageclass local-path -o wide")
    ...
    pod_details = n100_1.succeed("k3s kubectl get pods -n kube-system -l app=local-path-provisioner -o wide")
